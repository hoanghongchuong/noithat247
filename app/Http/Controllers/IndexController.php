...
class IndexController extends Controller {
	...
	*/
	...
	{		
		$tintuc = DB::table('news')->where('status',1)->where('com','tin-tuc')->orderby('id','desc')->paginate(12);		
	...
	public function getNewsDetail($id)
	{
		...
		if(!empty($news_detail)){			
			...
			$newsSameCate = DB::table('news')->where('status',1)->where('com','tin-tuc')
	...
	public function getListPost($alias)
	{
		...
		if(!empty($tintuc_cate)){
			...
			// End cấu hình SEO
			return view('templates.post_list', compact('tintuc','tintuc_cate','banner_danhmuc','keyword','description','title','img_share','tintuc_moinhat_detail','hot_news', 'cateNews'));
		}else{
			return redirect()->route('getErrorNotFount');
		}
	}
	...
	{
		$news_detail = DB::table('news')->select()->where('status',1)->where('com','post')->where('alias',$alias)->get()->first();		
		if(!empty($news_detail)){			
			$cate_pro = DB::table('product_categories')->where('status',1)->where('parent_id',0)->orderby('id','asc')->get();	
			$newsSameCate = DB::table('news')->where('status',1)->where('com','post')
			->where('id','<>',$news_detail->id)->where('cate_id',$news_detail->cate_id)->take(12)->get();		
			$com='tin-tuc';
			$setting = Cache::get('setting');
			// Cấu hình SEO
			if(!empty($news_detail->title)){
			...
			return view('templates.post_detail_tpl', compact('news_detail','com','keyword','description','title','img_share','newsSameCate'));
		}else{
			return redirect()->route('getErrorNotFount');
		}		
	}
